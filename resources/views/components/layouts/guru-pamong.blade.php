    ...
    <div class="flex min-h-screen">
        ...
        <aside class="w-64 bg-slate-800 text-white flex-shrink-0 flex flex-col">
            ...
            <div class="p-4 flex items-center gap-3 border-b border-slate-700">
                @if (auth()->user()->foto)
                    <img src="{{ Storage::url(auth()->user()->foto) }}" alt="Foto" class="w-10 h-10 rounded-full object-cover">
                @else
            ...
            <nav class="flex-1 overflow-y-auto">
                ...
                <a href="{{ route('guru-pamong.dashboard') }}"
                   class="flex items-center gap-3 px-4 py-3 text-sm hover:bg-slate-700 {{ request()->routeIs('guru-pamong.dashboard') ? 'bg-blue-600' : '' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" /></svg>
                    Dashboard
                </a>

                <a href="{{ route('guru-pamong.mahasiswa.index') }}"
                   class="flex items-center gap-3 px-4 py-3 text-sm hover:bg-slate-700 {{ request()->routeIs('guru-pamong.mahasiswa.*') ? 'bg-blue-600' : '' }}">
                ...
                <a href="{{ route('guru-pamong.logbook.index') }}"
                   class="flex items-center gap-3 px-4 py-3 text-sm hover:bg-slate-700 {{ request()->routeIs('guru-pamong.logbook.*') ? 'bg-blue-600' : '' }}">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                    Verifikasi Logbook
                </a>

                <a href="{{ route('guru-pamong.profil.index') }}"
                   class="flex items-center gap-3 px-4 py-3 text-sm hover:bg-slate-700 {{ request()->routeIs('guru-pamong.profil.*') ? 'bg-blue-600' : '' }}">
        ...
        <div class="flex-1 flex flex-col">
            ...
            <header class="bg-white shadow-sm px-6 py-4 flex justify-between items-center">
                ...
                <div class="text-sm text-gray-600">
                    {{ auth()->user()->name }} <span class="text-gray-400">- Guru Pamong</span>
                </div>
            </header>
